...
unzip gisaid_epiflu_sequence.fasta.zip
...
for i in Renamed_*_align.fas; do
  # Set output filename
  output="$(basename "$i" .fas)_output.fasta"

  # Check if the file is MP segment
  if [[ "$i" == "Renamed_MP_align.fas" ]]; then
    # Use second ATG option
    python -m vida.extract_cds -i "$i" -o "$output" -s
  else
    python -m vida.extract_cds -i "$i" -o "$output"
  fi

  # Build maximum-likelihood tree using FastTree
  fasttree -gtr -nt < "$output" > "$(echo $i | cut -d"_" -f2).nwk"
done
